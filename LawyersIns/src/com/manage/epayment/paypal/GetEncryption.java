...
public class GetEncryption 
{
	...
	private String 	paypalCertPath1;
	private String 	keyPass1;
	private static InetLogger logger = InetLogger.getInetLogger(GetEncryption.class);
			...
			CMSException 
	{
		...
		try (FileInputStream privateKeyInput = new FileInputStream(_privateKeyPath)) {
			ks.load(privateKeyInput, _keyPass.toCharArray());
		}
		...
		try (FileInputStream certificateInput = new FileInputStream(_certPath)) {
			certificate = (X509Certificate) cf.generateCertificate(certificateInput);
		}

		// Read the PayPal Cert
		X509Certificate payPalCert;
		try (FileInputStream payPalCertificateInput = new FileInputStream(_payPalCertPath)) {
			payPalCert = (X509Certificate) cf.generateCertificate(payPalCertificateInput);
		}
		...
		logger.debug("CMS payload size: " + baos.size());
	...
	public static byte[] DERtoPEM(byte[] bytes, String headfoot) 
	{
		...
		logger.debug("Converting " + stringBytes.length + " bytes");
