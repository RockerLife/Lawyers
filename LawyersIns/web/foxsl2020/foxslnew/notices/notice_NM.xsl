...
<xsl:stylesheet version="1.1"
	...
	xmlns:java="java"> 
      ...
      <xsl:include  href="group3/MO-42-002-04-17-NM.xsl" />      
       <xsl:include  href="group8/MO-42-002-04-19-NM.xsl" /> 
	  <xsl:template match="/">
		...
		<fo:root>		
		    ...
		    <fo:page-sequence master-reference="A4-portrait">		
				...
				<fo:flow flow-name="xsl-region-body">
					<xsl:if test="response/NMRatingVersion = 'NM1'">
						<fo:block>															
								<xsl:call-template name="NM-DWL-Notice" />											
				   		</fo:block>		
						</xsl:if>	
					<xsl:if test="response/NMRatingVersion = 'NM2'">
						<fo:block>															
							<xsl:call-template name="NM-DWL-Notice_NM2" />											
				  	 	</fo:block>		
